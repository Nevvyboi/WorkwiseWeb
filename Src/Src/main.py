...
from Utils.emailUtil import send_password_reset_email, send_password_changed_confirmation, generate_reset_code
...
from Models.models import (
    ...
    UnionIn, UnionOut, UnionMemberIn, UnionMemberOut, JobOut,
    ForgotPasswordIn, ForgotPasswordOut, VerifyResetCodeIn, VerifyResetCodeOut,
...
from Database.db import (
    ...
    unionExists, getUnions, workerInUnion, getUnionMembers, getRandomJobs, getJobById,
    createPasswordResetToken, getUserByEmail, verifyResetCode,
    markResetTokenAsUsed, updateUserPassword, cleanupExpiredResetTokens
)
...
endpointTokens = {
    ...
    # Jobs
    "GET:/v1/workwise/jobs": "REDACTED-ENDPOINT-TOKEN",
    "GET:/v1/workwise/jobs/{job_id}": "REDACTED-ENDPOINT-TOKEN",

    "POST:/v1/workwise/forgot-password": "REDACTED-ENDPOINT-TOKEN",
    "POST:/v1/workwise/verify-reset-code": "REDACTED-ENDPOINT-TOKEN",
    "POST:/v1/workwise/reset-password": "REDACTED-ENDPOINT-TOKEN",
}
...
def getProfile(user_id: int):
    ...
    try:
        ...
        profile_image = user.get("profile_image") if "profile_image" in user else None
        profile_name = user.get("profile_name") if "profile_name" in user else None
        profile_bio = user.get("profile_bio") if "profile_bio" in user else None
        profile_number = user.get("phone_number") if "phone_number" in user else None
        profile_location = user.get("location") if "location" in user else None
        updated_at = user.get("updated_at") if "updated_at" in user else None
...
def listJobs(limit: int = 20):
    conn = getDatabase()
    try:
        jobs = getRandomJobs(conn, limit=limit)
        return [
            JobOut(
                jobId=j["job_id"],
...
@app.get(
    "/v1/workwise/jobs/{job_id}",
    response_model=JobOut,
    tags=["jobs"],
    dependencies=[Depends(requireEndpointToken(endpointTokens[key("GET", "/v1/workwise/jobs/{job_id}")]))]
)
...
def forgotPassword(body: ForgotPasswordIn):
    """
    ...
    try:
        # Find user by email
        user = getUserByEmail(conn, body.email)
        ...
        token_id = createPasswordResetToken(conn, user["user_id"], body.email, reset_code)
        
        if not token_id:
            raise HTTPException(status_code=500, detail="Failed to create reset token")
        
        # Send email with reset code
        email_sent = send_password_reset_email(body.email, reset_code, user["username"])
        
        if not email_sent:
            raise HTTPException(status_code=500, detail="Failed to send reset email")
        
        # Clean up old tokens (optional maintenance)
        cleanupExpiredResetTokens(conn)
...
def verifyResetCodeEndpoint(body: VerifyResetCodeIn):
    """
    ...
    try:
        ...
        token = verifyResetCode(conn, body.email, body.resetCode)
        
        if not token:
            return VerifyResetCodeOut(
...
def resetPassword(body: ResetPasswordIn):
    """
    ...
    try:
        ...
        # Hash new password
        new_password_hash = pwd.hash(body.newPassword)
        
        # Update password
        if not updateUserPassword(conn, user["user_id"], new_password_hash):
            raise HTTPException(status_code=500, detail="Failed to update password")
        
        # Mark token as used
        markResetTokenAsUsed(conn, token["token_id"])
        
        # Send confirmation email
        send_password_changed_confirmation(body.email, user["username"])
...
@app.get("/v1/workwise/forgot-password", tags=["auth"])
def forgotPasswordProbeHtml():
    raise HTTPException(status_code=401, detail="Missing or invalid endpoint token")
